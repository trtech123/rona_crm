...
import {
    ...
    Globe, Facebook, Instagram, Linkedin, Twitter, MessageSquare
} from "lucide-react";
...
export default function PostsDashboardPage() { // Renamed component conceptually
    ...
    const [selectedPostForView, setSelectedPostForView] = useState<PostDisplayData | null>(null);
    ...
    useEffect(() => {
        const fetchPosts = async () => { // Renamed function
            ...
            try {
                ...
                const processedData = data?.map((post): PostDisplayData => {
                    const platformKey = post.platform?.toLowerCase() || 'other';
                    const platformStyle = platformStyleMapping[platformKey] || platformStyleMapping['other'];
                    return {
    ...
    // --- Filtering Logic ---
    const filteredPosts = useMemo(() => { // Renamed variable
        return posts.filter(post => {
            const searchTermMatch = filters.searchTerm === '' ||
                post.content?.toLowerCase().includes(filters.searchTerm.toLowerCase()) || // Search content
            ...
            const platformMatch = filters.platform.length === 0 || (post.platform ? filters.platform.includes(post.platform) : false);
    ...
    const handleSortChange = (newSortBy: string) => {
         ...
         const validSortColumns = ['content', 'platform', 'created_at', 'updated_at'];
    ...
    const getDistinctPlatforms = (): string[] => {
        return Array.from(new Set(posts.map(post => post.platform).filter(value => value != null) as string[]));
    };
                         ...
                         <div className="flex flex-wrap items-center gap-3">
                             ...
                             <DropdownMenu>
                                 ...
                                 <DropdownMenuContent align="start">
                                     {getDistinctPlatforms().map((platform) => ( // Use distinct platforms
                                         <DropdownMenuCheckboxItem
                                             key={platform}
                                             checked={filters.platform.includes(platform)}
                                             onCheckedChange={() => handleCheckboxFilterChange(platform)} // Updated handler call
                                         >
                                             {platform}
                                         </DropdownMenuCheckboxItem>
                                     ))}
                ...
                {/* Loading and Error States */}
                 {isLoading && (
                     <div className="flex justify-center items-center py-10">
                         ...
                         <p className="ml-3 text-purple-700">טוען פוסטים...</p> {/* Updated text */}
                     </div>
                 )}
                 {error && (
                 ...
                 {!isLoading && !error && (
                     <>
                         {filteredPosts.length === 0 ? (
                             <div className="text-center py-10 text-gray-500"><p>לא נמצאו פוסטים התואמים את הסינון.</p></div>
                         ) : viewMode === 'grid' ? (
                             // Grid View - Use imported PostGridCard component
                             <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                                 {filteredPosts.map(post => ( // Iterate over filteredPosts
                                ...
                                <div className="overflow-x-auto">
                                    <table className="min-w-full divide-y divide-gray-200">
                                        <thead className="bg-gray-50">
                                            <tr>
                                                 <th scope="col" className="px-3 py-2 text-right text-xs font-medium text-gray-500 uppercase tracking-wider cursor-pointer" onClick={() => handleSortChange('content')}><div className="flex items-center">תוכן {renderSortIcon('content')}</div></th>
                                                 <th scope="col" className="px-3 py-2 text-right text-xs font-medium text-gray-500 uppercase tracking-wider cursor-pointer" onClick={() => handleSortChange('platform')}><div className="flex items-center">פלטפורמה {renderSortIcon('platform')}</div></th>
                                                  <th scope="col" className="px-3 py-2 text-right text-xs font-medium text-gray-500 uppercase tracking-wider cursor-pointer" onClick={() => handleSortChange('created_at')}><div className="flex items-center">תאריך יצירה {renderSortIcon('created_at')}</div></th>
                                                  <th scope="col" className="px-3 py-2 text-right text-xs font-medium text-gray-500 uppercase tracking-wider cursor-pointer" onClick={() => handleSortChange('updated_at')}><div className="flex items-center">תאריך עדכון {renderSortIcon('updated_at')}</div></th>
                                                  <th scope="col" className="px-3 py-2 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">פעולות</th>
                                             </tr>
                                         </thead>
                                          <tbody className="bg-white divide-y divide-gray-100">
                                              {filteredPosts.map(post => ( // Iterate over filteredPosts
                                                  <PostListRow key={post.id} post={post} onViewPost={handleViewPost} /> // Pass post and handler
                                              ))}
                                          </tbody>
                                     </table>
                                 </div>
